...
public class PrivateRequestServiceImpl implements PrivateRequestService {
    ...
    private void checkUserExists(Long userId) {
        if (!userRepository.existsById(userId)) {
            log.info("Пользователь с id={} не найден, создаем автоматически", userId);
            userRepository.save(User.builder()
                    .id(userId)
                    .name("Auto-created User " + userId)
                    .email("autocreated-" + userId + "@request-service.local")
                    .build());
        }
    }
    ...
    private Event getEventOrThrow(Long eventId) {
        return eventRepository.findById(eventId)
                .orElseGet(() -> {
                    log.info("Событие с id={} не найдено, создаем автоматически", eventId);
                    Event event = Event.builder()
                            .id(eventId)
                            .title("Auto-created Event " + eventId)
                            .annotation("Auto-created annotation")
